...
import { Check, Download, ChevronDown, FileText, ArrowRight, GraduationCap, MessageCircle } from "lucide-react";
import {
  motion, useScroll, useTransform, useSpring, useInView, AnimatePresence
} from "framer-motion";
import { whatsappLink, getLocation, locationLink, type GeoCoords } from "@/lib/whatsapp";

export const Route = createFileRoute("/admissions")({
  head: () => ({
    meta: [
      { title: "Admissions — Preeminence International Montessori" },
      { name: "description", content: "Apply to Preeminence. Step-by-step admissions, fees and FAQ for Nursery & Primary classes." },
      { property: "og:title", content: "Admissions at Preeminence" },
      { property: "og:description", content: "Admissions for Nursery & Primary at Preeminence International Montessori." },
    ],
  }),
  ...
  return (
    <section ref={ref} className="relative overflow-hidden pb-24 min-h-[500px] flex items-center bg-white">
      <motion.div style={{ y: yBg, scale: scaleBg }} className="absolute inset-0 origin-center">
        <img src={HERO_IMG} alt="Admissions" className="absolute inset-0 h-full w-full object-cover opacity-70" />
      </motion.div>
      ...
      <motion.div style={{ y: yContent, opacity }}
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
          ...
          <span className="text-[11px] font-medium tracking-widest uppercase text-blue-700">Admissions</span>
        </motion.div>
        <motion.h1 initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.1 }}
          ...
          Begin your<br />
          <em className="not-italic text-[#C21E1E]">Preeminence story.</em>
        </motion.h1>
        <motion.p initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }}
          className="text-base md:text-lg text-slate-600 max-w-2xl mx-auto leading-relaxed">
          A simple, friendly journey into our bright, caring classrooms.
        </motion.p>
      </motion.div>
    </section>
  );
...
function Steps() {
  const steps = [
    ...
    { t: "Assess",   d: "A friendly readiness check and a short chat with your child.", num: "04" },
    ...
    <section className="py-24 bg-warm-white">
      <div className="mx-auto max-w-7xl px-6">
        <div className="text-center mb-14">
          <SectionLabel label="The Process" light={false} />
          <FadeUp>
            <h2 className="font-display text-4xl md:text-5xl font-bold text-blue-900">
              Six steps to <em className="not-italic text-[#C21E1E]">joining us.</em>
            </h2>
          </FadeUp>
        </div>
        ...
        <StaggerList className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {steps.map((s) => (
            <motion.div key={s.t} variants={staggerItem}
              className="relative rounded-2xl bg-white border border-slate-200 p-6 hover:-translate-y-2 hover:border-blue-300 hover:shadow-md transition-all duration-300 group overflow-hidden">
              <div className="absolute top-4 right-4 font-display font-bold text-5xl text-blue-100 select-none group-hover:text-blue-200 transition-colors">
                {s.num}
              </div>
              <div className="h-8 w-8 rounded-lg bg-blue-100 border border-blue-200 flex items-center justify-center mb-4">
                <span className="text-xs font-bold text-blue-600">{s.num}</span>
              </div>
              <h3 className="font-display font-bold text-xl text-blue-900 mb-2">{s.t}</h3>
...
function Fees() {
  const fees = [
    { g: "Nursery 1–2",            a: "₦20,000", e: "₦5,000" },
    { g: "Primary 1–3",            a: "₦25,000", e: "₦7,500" },
    { g: "Primary 4–6",            a: "₦30,000", e: "₦10,000" },
  ];
  ...
  return (
    <section className="py-24 bg-warm-white">
      <div className="mx-auto max-w-5xl px-6">
        <div className="text-center mb-14">
          ...
          <FadeUp>
            <h2 className="font-display text-4xl md:text-5xl font-bold text-slate-900">
              Transparent <em className="not-italic text-[#C21E1E]">pricing.</em>
            </h2>
          </FadeUp>
          <FadeUp delay={0.08}>
        ...
        <FadeUp delay={0.05}>
          <div className="rounded-2xl bg-warm-white border border-slate-200 shadow-sm overflow-hidden">
            {/* Header */}
            <div className="grid grid-cols-3 gap-4 px-6 py-4 bg-slate-50 border-b border-slate-200">
              {["Class Level", "Annual Tuition", "Enrollment Fee"].map((h) => (
...
function FAQ() {
  ...
  const faqs = [
    ...
    { q: "What ages do you accept?",                    a: "We welcome children from Nursery 1 through Primary 6 — roughly ages 3 to 11 — into mixed-age, family-like classrooms." },
    { q: "What languages of instruction are available?",    a: "English-medium, with our local languages and French introduced gently through songs, games and stories." },
    { q: "Can international families apply?",               a: "Absolutely. We warmly welcome international families with dedicated support for a smooth, caring transition." },
  ];
    ...
    <section className="py-24 bg-white">
      <div className="mx-auto max-w-3xl px-6">
        <div className="text-center mb-14">
          ...
          <FadeUp>
            <h2 className="font-display text-4xl md:text-5xl font-bold text-blue-900">
              Frequently <em className="not-italic text-[#C21E1E]">asked.</em>
            </h2>
          </FadeUp>
        </div>
        ...
        <StaggerList className="space-y-3" stagger={0.07}>
          {faqs.map((f, i) => (
            <motion.div key={f.q} variants={staggerItem}
              className="rounded-2xl bg-warm-white border border-slate-200 overflow-hidden hover:border-blue-300 transition-colors">
...
function InquirySection() {
  ...
  const handleSubmit = () => {
    ...
    const text = [
      "New admissions inquiry from the Preeminence website:",
      "",
      `Parent / Guardian Name: ${form.parentName}`,
      `Email: ${form.email}`,
      `Class Applying For: ${form.grade}`,
      ...(form.message ? ["", `About the child: ${form.message}`] : []),
    ...
    <section className="py-24 bg-warm-white">
      <div className="mx-auto max-w-5xl px-6">
        <div className="text-center mb-14">
          ...
          <FadeUp>
            <h2 className="font-display text-4xl md:text-5xl font-bold text-slate-900">
              Take the first <em className="not-italic text-[#C21E1E]">step.</em>
            </h2>
          </FadeUp>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {/* Inquiry form */}
          <FadeFrom dir="left">
            <div className="rounded-2xl bg-warm-white border border-slate-200 shadow-sm p-7 h-full">
              ...
              {submitted ? (
                ...
                <div className="grid gap-3">
                  ...
                  <label className="flex items-start gap-3 rounded-xl bg-slate-50 border border-slate-200 px-4 py-3 cursor-pointer select-none">
                    <input
                      type="checkbox"
                      checked={shareLoc}
                    ...
                    <div className="text-sm">
                      <div className="font-medium text-slate-800">Share my location</div>
                      <div className="text-slate-500 text-xs">
                        {locating
                          ? "Detecting location…"
                          : locError
                            ? "Location unavailable — we'll send your inquiry without it."
                            : location
                              ? "Location ready — it'll be added to your inquiry."
                  ...
                  <button
                    ...
                    disabled={!isValid}
                    className="rounded-xl bg-yellow-400 text-blue-900 hover:bg-yellow-300 disabled:opacity-40 disabled:cursor-not-allowed  py-3 font-semibold text-sm transition-colors flex items-center justify-center gap-2 shadow-sm">
          ...
          <FadeFrom dir="right">
            <div className="flex flex-col gap-5">
              <div className="rounded-2xl bg-warm-white border border-slate-200 shadow-sm p-7">
              ...
              <FadeUp delay={0.1}>
                <div className="rounded-2xl bg-emerald-50 border border-emerald-200 p-5 flex items-center gap-3">
                  ...
                  <span className="text-sm font-medium text-emerald-800 leading-snug">
                    Rolling admissions for Nursery and Primary classes are open.
                  </span>
                </div>
              </FadeUp>
            </div>
          ...
          <div className="relative">
            <FadeUp>
              ...
              <h2 className="font-display text-4xl md:text-5xl font-bold text-blue-900 mb-4">
                Ready to apply?<br />
                <em className="not-italic text-[#C21E1E]">We're ready for you.</em>
              </h2>
            </FadeUp>
            <FadeUp delay={0.1}>
              <p className="text-base text-slate-600 max-w-xl mx-auto leading-relaxed mb-10">
                Applications for the 2025–2026 academic year are now open. Join a warm community of curious young learners and their families.
              </p>
              <div className="flex flex-wrap items-center justify-center gap-3">
                <a
                  href={whatsappLink("Hello! I'd like to start my application to Preeminence International Montessori.")}
                  target="_blank"
