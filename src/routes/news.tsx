  ...
  return (
    <section ref={ref} className="relative overflow-hidden pb-24 min-h-[500px] flex items-center bg-blue-700">
      <motion.div style={{ y: yBg, scale: scaleBg }} className="absolute inset-0 origin-center">
        <img src={IMGS.hero} alt="Newsroom" className="absolute inset-0 h-full w-full object-cover" />
      </motion.div>
      <div className="absolute inset-0 bg-gradient-to-r from-blue-700/95 via-blue-700/85 to-blue-600/70" />
      ...
      <motion.div style={{ y: yContent, opacity }}
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
          className="inline-flex items-center gap-2 rounded-full border border-blue-600/40 bg-blue-700/80 px-4 py-1.5 mb-6">
          <span className="h-1.5 w-1.5 rounded-full bg-blue-400" />
          <span className="text-[11px] font-medium tracking-widest uppercase text-blue-300/80">Newsroom</span>
        </motion.div>
        <motion.h1 initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.1 }}
          ...
          Stories from<br />
          <em className="not-italic text-blue-400">the College.</em>
        </motion.h1>
...
══════════════════════════════════════════════════ */
const catStyle = {
  Academic:  { badge: "bg-blue-900/60 text-blue-300 border-blue-700/40",          bar: "bg-blue-500"    },
  Community: { badge: "bg-emerald-900/60 text-emerald-300 border-emerald-700/40", bar: "bg-emerald-500" },
};
...
function FeaturedStory({ post }) {
  ...
  return (
    <section ref={sectionRef} className="py-16 bg-blue-950">
      <div className="mx-auto max-w-7xl px-6">
        <SectionLabel label="Featured Story" light={true} />
        <div className="rounded-3xl overflow-hidden grid lg:grid-cols-2 bg-blue-900/40 border border-blue-800/40">
          ...
          <div className="p-8 md:p-12 flex flex-col justify-center">
            <FadeUp delay={0.05}>
              <div className="flex items-center gap-2 text-xs text-blue-400/60 mb-4">
                <Calendar className="h-3 w-3" /> {post.d}
              </div>
              <h2 className="font-display text-2xl md:text-4xl font-bold text-white leading-snug mb-4">
                {post.t}
              </h2>
              <p className="text-base text-blue-200/55 leading-relaxed mb-8">
    ...
    <section className="py-16 bg-slate-900">
      <div className="mx-auto max-w-7xl px-6">
        <div className="text-center mb-12">
          <SectionLabel label="All Stories" light={true} />
          <FadeUp>
            <h2 className="font-display text-4xl md:text-5xl font-bold text-white">
              What's been <em className="not-italic text-blue-400">happening.</em>
            </h2>
          </FadeUp>
        </div>
              ...
              <button key={k} onClick={() => setActiveCat(k)}
                className={`rounded-full px-4 py-1.5 text-xs font-medium transition-colors ${
                  activecat === k
                    ...
                    : "bg-blue-700/70 border border-blue-600/70 text-blue-300/70 hover:text-blue-200 hover:border-blue-600/50"
                }`}>
                {k}
              </button>
            ))}
            ...
            {filtered.map((p, i) => {
              ...
              return (
                <motion.article key={p.t}
                  ...
                  className="group rounded-2xl overflow-hidden bg-blue-900/40 border border-blue-800/40 hover:border-blue-600/50 hover:-translate-y-1 transition-all duration-300 cursor-pointer"
                >
                  ...
                  <div className="p-5">
                    ...
                    <div className="flex items-center gap-1.5 text-xs text-blue-400/50 mb-2">
                      <Calendar className="h-3 w-3" /> {p.d}
                    </div>
                    <h3 className="font-display font-bold text-lg text-white group-hover:text-blue-300 transition-colors leading-snug mb-3">
                      {p.t}
                    </h3>
        ...
        {filtered.length === 0 && (
          <FadeUp className="py-20 text-center">
            <p className="text-blue-400/50 text-sm">No stories match your search.</p>
          </FadeUp>
        )}
      </div>
    </section>
...
function OpenHouseBanner() {
  ...
  return (
    <section ref={sectionRef} className="py-24 bg-blue-950">
      <div className="mx-auto max-w-5xl px-6">
        <motion.div ref={ref}
          initial={{ opacity: 0, scale: 0.96, y: 24 }}
          animate={inView ? { opacity: 1, scale: 1, y: 0 } : { opacity: 0, scale: 0.96, y: 24 }}
          transition={{ duration: 0.65, ease: [0.25, 0.46, 0.45, 0.94] }}
          className="relative rounded-3xl bg-blue-800 border border-blue-700/50 overflow-hidden">
          ...
          <div className="relative p-8 md:p-12 grid md:grid-cols-[1fr_auto] gap-6 items-center">
            <div>
              <FadeUp>
                <div className="inline-flex items-center gap-2 rounded-full border border-blue-500/30 bg-blue-700/30 px-3 py-1 mb-4">
                  <span className="h-1.5 w-1.5 rounded-full bg-blue-300 animate-pulse" />
                  <span className="text-[10px] uppercase tracking-widest text-blue-300/80 font-medium">Event Registration</span>
                </div>
                <h3 className="font-display text-2xl md:text-3xl font-bold text-white mb-2">
                  Open House — June 14, 2026
                </h3>
                <p className="text-blue-200/60 text-sm leading-relaxed max-w-lg">
            ...
            <FadeFrom dir="right" delay={0.1}>
              <button className="rounded-xl bg-white text-blue-700 hover:bg-blue-50 px-6 py-3 font-bold text-sm transition-colors whitespace-nowrap flex items-center gap-2">
